...
class PolicyNet(nn.Module):
    def __init__(self, n_states, n_actions):
        ...
        # print('Ploci',n_states,n_actions)

        self.in_put = nn.Linear(n_states, n_hiddens)

        # self.fc1 = nn.Linear(n_hiddens, n_hiddens)
        # self.fc2 = nn.Linear(n_hiddens, n_hiddens)
        
        self.out_put = nn.Linear(n_hiddens, n_actions)
    def forward(self, x):
        ...
        x = x.float()
        # print('Poli for',x.dtype)
        x = self.in_put(x)  # [b,n_states]-->[b,n_hiddens]
        x = F.relu(x)
        ...
        # x = F.relu(x)
        # x = self.fc2(x)  # [b,n_states]-->[b,n_hiddens]
        # x = F.relu(x)
